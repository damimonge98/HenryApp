...
router.post('/', async(req, res) => {
  const { enterprise, title, description, remote, location, linkedIn } = req.body;
...
router.get('/:id', async(req, res)=>{
  const {id} = req.params;
  Empleo.findById(id).then(empleo=>{
    if(!empleo){
      return res.status(404).json({message: 'El empleo solicitado no existe'});
